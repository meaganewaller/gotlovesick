...
const Page = async ({ params, searchParams }: Props) => {
  ...
  if (data.recommendations?.pageInfo?.offsetPagination?.total) {
    totalPages = Math.ceil(data.recommendations.pageInfo.offsetPagination.total / perPage)
  }
            ...
            {recommendations?.map((rec: any) => (
              <div key={rec.id} className='card'>
                <div className='card__intro'>
                  <Link href={`/recs/${typeSlug}/${rec.slug}`} className="card__heading">{rec.title}</Link>
                </div>
              </div>
            ))}
          </section>
