      ...
      <div self="size-x3 sm-first" className="bg-fuchsia-100 border-fuchsia-600 border-solid border-2 max-h-[65vh] overflow-scroll">
        <main className="flex flex-col">
          <article className="flex">
            <h1 className="self-center text-5xl font-jotted font-normal text-fuchsia-400 px-20 text-center" dangerouslySetInnerHTML={{ __html: homepage.title }} />
            <Image src={glitterkitty} alt="" width={250} className="align-middle" />
          </article>
          <div>
            <h2 className='font-cute text-4xl font-normal'>Recent Blog Posts</h2>
            {posts.map((post: Post) => (
              <article key={post.databaseId}>
                <Link href={`/blog/${post.slug}`}>
                  <h2
                    className="text-xl my-5 hover:italic"
                    dangerouslySetInnerHTML={{ __html: post.title }} />
                </Link>
              </article>
            ))}

            <hr className="border-fuchsia-500 my-10" />

            <h2 className='font-cute text-4xl font-normal'>Recent Bookmarks</h2>
