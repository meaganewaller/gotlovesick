  ...
  return (
    ...
    >
      {item.questionRelativeToImage === 'adjacent' && (
        <h2
          className="question-adjacent-text"
          style={{
            color: item.fontColor
              ? item.fontColor
              : generalFontColor
      ...
      {item.backgroundImageSrc ? (
        <div
          className={`question-image-container ${item.questionRelativeToImage === 'adjacent' ? 'question-adjacent-to-image' : ''} ${item.imageAttribution ? 'image-attribution' : ''}`}
          ref={resizeRef}
        >
          <Image
