...
function TutorialGridCard({ item }: TutorialCardProps) {
  return (
    <div className={classNames("tutorial-card")} key={item.id}>
      <figure>
        {item.featuredImage && <Image src={item.featuredImage.node.sourceUrl} alt={item.featuredImage.node.altText || ''} width={item.featuredImage.node.mediaDetails.width} height={item.featuredImage.node.mediaDetails.height} />}
      </figure>
      <div className="meta">
      ...
      <Link href={`/tutorials/${item.slug}`}><h2>{item.title}</h2></Link>
      <div dangerouslySetInnerHTML={{ __html: item.tutorialFields.description  }} />
    </div>
  )
}
  ...
  if (items && items.length > 0) {
    return (
      <div id="tutorialsGrid" ref={masonryContainer}>
        {items.map((item, index: number) => ( <TutorialGridCard key={item.id} item={item} index={index} />))}
      </div>
    )
  } else {
    return (
