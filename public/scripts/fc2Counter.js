if (!self._FC2COUNTER89509967_0) {
  var _FC2COUNTER89509967_0 = {
    id: 89509967,
    graph_img: null,
    show_graph: function (e) {
      if (this.graph_img == null) {
        ...
        img.src =
          'https://counter1-cdn-ssl.fc2.com/popup.php?id=89509967&main=0&lang=0'
    ...
    visit_info: function () {
      ...
      var reg = (' ' + document.cookie + ';').match(
        new RegExp(' ' + cookie_name + '=([\\d\\-]+);')
      )
      if (reg) {
        info = reg[1].split('-')
        is_first = false
      ...
      document.cookie =
        cookie_name +
        '=' +
        [version, is_unique ? now : info[1]].join('-') +
        ' ; expires=' +
        time.toGMTString()
